...
// packages/course-contract/src/assets.ts
function isRelativeAsset(p) {
  return !!p && !/^https?:\/\//i.test(p) && !p.startsWith("data:");
}
function collectAssetPaths(document) {
...
// packages/course-contract/src/primitives.ts
var idSchema = external_exports.string().regex(/^[a-z0-9]+(-[a-z0-9]+)*$/, "must be a lower-case hyphenated id");
var relativeAssetPathSchema = external_exports.string().min(1).refine((p) => !p.startsWith("/"), "must not be absolute").refine((p) => !p.includes("\\"), "must not contain backslashes").refine((p) => !/^[a-z][a-z0-9+.-]*:\/\//i.test(p), "must not contain a URL scheme").refine((p) => !p.split("/").includes(".."), "must not contain a parent-directory segment");
var courseIdSchema = idSchema;
var objectiveIdSchema = idSchema;
var partIdSchema = idSchema;
var sliceIdSchema = idSchema;
var blockIdSchema = idSchema;
var narrationIdSchema = idSchema;
var workflowStepIdSchema = idSchema;

// packages/course-contract/src/blocks.ts
var ChoiceOption = external_exports.object({ id: external_exports.string().min(1), label: external_exports.string().min(1) }).strict();
var SingleChoiceAssessment = external_exports.discriminatedUnion("mode", [
  external_exports.object({
    mode: external_exports.literal("graded"),
    correctOptionId: external_exports.string().min(1),
    correctFeedback: external_exports.string().optional(),
    incorrectFeedback: external_exports.string().optional()
  }).strict(),
  external_exports.object({ mode: external_exports.literal("survey") }).strict()
]);
var FillBlankAssessment = external_exports.discriminatedUnion("mode", [
  external_exports.object({
    mode: external_exports.literal("graded"),
    acceptedAnswers: external_exports.array(external_exports.string().min(1)).min(1),
    caseSensitive: external_exports.boolean().optional(),
    correctFeedback: external_exports.string().optional(),
    incorrectFeedback: external_exports.string().optional()
  }).strict(),
  external_exports.object({ mode: external_exports.literal("reflection"), rubric: external_exports.string().min(1) }).strict()
]);
var submitAny = external_exports.object({ rule: external_exports.literal("submit-any") }).strict();
var submitCorrect = external_exports.object({ rule: external_exports.literal("submit-correct") }).strict();
...
var VideoBlock = external_exports.object({
  ...
  interaction: external_exports.object({ source: relativeAssetPathSchema }).strict().optional(),
...
var FillBlankBlock = external_exports.object({
  id: blockIdSchema,
  type: external_exports.literal("fillBlank"),
...
var SingleChoiceBlock = external_exports.object({
  id: blockIdSchema,
  type: external_exports.literal("singleChoice"),
  prompt: external_exports.string().min(1),
...
// packages/course-contract/src/htmlMessages.ts
var HtmlReadyPayload = external_exports.record(external_exports.string(), external_exports.unknown()).optional();
var HtmlProgressPayload = external_exports.record(external_exports.string(), external_exports.unknown()).optional();
var HtmlErrorPayload = external_exports.object({
  message: external_exports.string().min(1),
  code: external_exports.string().optional()
}).strict();
var HtmlCompletedPayload = external_exports.object({
  /** The frame's own stable id for THIS completion, for resend detection. Never used as the host-side interaction identity. */
  resultId: external_exports.string().min(1).optional(),
  correct: external_exports.boolean().optional(),
  value: external_exports.unknown().optional()
}).strict().refine((p) => p.correct !== void 0 || p.value !== void 0, {
  message: "completed payload must carry at least one learning-evidence field (correct and/or value)"
});

// packages/course-contract/src/videoInteraction.ts
var SingleChoiceActivity = external_exports.object({
  type: external_exports.literal("singleChoice"),
  options: external_exports.array(ChoiceOption).min(2),
  assessment: SingleChoiceAssessment,
  completion: SingleChoiceCompletionRule
}).strict();
var FillBlankActivity = external_exports.object({
  type: external_exports.literal("fillBlank"),
  assessment: FillBlankAssessment,
  completion: FillBlankCompletionRule
}).strict();
var VideoInteractionActivity = external_exports.discriminatedUnion("type", [SingleChoiceActivity, FillBlankActivity]);
var VideoInteractionCue = external_exports.object({
  id: external_exports.string().min(1),
  atSeconds: external_exports.number().nonnegative(),
  pauseVideo: external_exports.boolean(),
  required: external_exports.boolean(),
  prompt: external_exports.string().min(1),
  activity: VideoInteractionActivity
}).strict();
var VideoInteractionDocument = external_exports.object({
  schemaVersion: external_exports.literal("1.1"),
  video: external_exports.object({
    blockId: blockIdSchema,
    source: relativeAssetPathSchema,
    durationSeconds: external_exports.number().positive(),
    cues: external_exports.array(VideoInteractionCue)
  }).strict()
}).strict();
...
var LayoutDefinition = external_exports.object({ preset: LayoutPreset, ratio: SplitRatio.optional(), slots: external_exports.array(LayoutSlot).min(1) }).strict().superRefine((layout, ctx) => {
  ...
  const has = (want) => want.length === ids.length && want.every((w) => ids.includes(w));
...
// packages/course-contract/src/narration.ts
var NarrationDefinition = external_exports.object({ id: narrationIdSchema, text: external_exports.string().min(1), audio: relativeAssetPathSchema, durationSeconds: external_exports.number().positive().optional() }).strict();

// packages/course-contract/src/navigation.ts
var NavigationDefinition = external_exports.object({
  previous: external_exports.literal("allowed"),
  manualNext: external_exports.enum(["after-completion", "allowed"]),
  autoNext: external_exports.boolean(),
  revisit: external_exports.literal("restore-completed-state")
}).strict();

// packages/course-contract/src/workflow.ts
var TargetRef = external_exports.union([
  external_exports.object({ blockId: blockIdSchema }).strict(),
  external_exports.object({ blockId: blockIdSchema, itemId: external_exports.string().min(1) }).strict()
]);
var WorkflowAction = external_exports.discriminatedUnion("type", [
  external_exports.object({ type: external_exports.literal("show"), targetId: blockIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("hide"), targetId: blockIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("focus"), target: TargetRef }).strict(),
  external_exports.object({ type: external_exports.literal("clearFocus") }).strict(),
  external_exports.object({ type: external_exports.literal("enable"), targetId: blockIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("disable"), targetId: blockIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("playNarration"), narrationId: narrationIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("pauseNarration"), narrationId: narrationIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("stopNarration"), narrationId: narrationIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("playBlock"), targetId: blockIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("pauseBlock"), targetId: blockIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("resetBlock"), targetId: blockIdSchema }).strict(),
  external_exports.object({ type: external_exports.literal("startTimer"), timerId: external_exports.string().min(1), durationSeconds: external_exports.number().positive() }).strict(),
  external_exports.object({ type: external_exports.literal("cancelTimer"), timerId: external_exports.string().min(1) }).strict(),
  external_exports.object({ type: external_exports.literal("completeSlice") }).strict(),
  external_exports.object({ type: external_exports.literal("navigate"), target: external_exports.literal("nextSlice") }).strict()
]);
var WorkflowEventType = external_exports.enum([
  "narration.ended",
  "video.started",
  "video.paused",
  "video.ended",
...
var WorkflowEventMatcher = external_exports.object({
  type: WorkflowEventType,
  sourceId: external_exports.string().min(1).optional(),
  interactionId: external_exports.string().min(1).optional(),
  timerId: external_exports.string().min(1).optional()
}).strict();
var WorkflowTransition = external_exports.object({ on: WorkflowEventMatcher, to: workflowStepIdSchema }).strict();
var WorkflowStep = external_exports.object({ id: workflowStepIdSchema, enterActions: external_exports.array(WorkflowAction), transitions: external_exports.array(WorkflowTransition) }).strict();
var SliceInitialState = external_exports.object({
  visibleBlockIds: external_exports.array(blockIdSchema).optional(),
  enabledBlockIds: external_exports.array(blockIdSchema).optional(),
  focusedTarget: TargetRef.optional()
}).strict();
var SliceWorkflow = external_exports.object({
  version: external_exports.literal("1.0"),
  initialStepId: workflowStepIdSchema,
  initialState: SliceInitialState.optional(),
  steps: external_exports.array(WorkflowStep).min(1)
}).strict();

// packages/course-contract/src/course.ts
var CourseObjective = external_exports.object({ id: objectiveIdSchema, text: external_exports.string().min(1), evidenceBlockIds: external_exports.array(blockIdSchema).min(1) }).strict();
var OpeningSignal = external_exports.enum(["recent-course-topics", "prior-objective-performance"]);
var OpeningDefinition = external_exports.object({
  learningPreview: external_exports.array(external_exports.string().min(1)),
  personalization: external_exports.object({ enabled: external_exports.boolean(), allowedSignals: external_exports.array(OpeningSignal) }).strict(),
  fallback: external_exports.object({ text: external_exports.string().min(1), audio: relativeAssetPathSchema.optional() }).strict()
}).strict();
var ClosingSignal = external_exports.enum(["answers", "attempts", "time-on-slice", "interaction-results"]);
var ClosingDefinition = external_exports.object({
  preparedSummary: external_exports.string().min(1),
  takeaways: external_exports.array(external_exports.string().min(1)),
  transferApplications: external_exports.array(external_exports.string().min(1)),
  personalization: external_exports.object({ enabled: external_exports.boolean(), allowedSignals: external_exports.array(ClosingSignal) }).strict(),
  fallback: external_exports.object({ text: external_exports.string().min(1), audio: relativeAssetPathSchema.optional() }).strict()
}).strict();
var SliceDefinition = external_exports.object({
  id: sliceIdSchema,
  title: external_exports.string().min(1),
  objectiveIds: external_exports.array(objectiveIdSchema),
  estimatedSeconds: external_exports.number().positive(),
  blocks: external_exports.array(BlockDefinition).min(1),
  layout: LayoutDefinition,
  narrations: external_exports.array(NarrationDefinition),
  workflow: SliceWorkflow,
  navigation: NavigationDefinition
}).strict();
var PartDefinition = external_exports.object({ id: partIdSchema, title: external_exports.string().min(1), objectiveIds: external_exports.array(objectiveIdSchema), slices: external_exports.array(SliceDefinition).min(1) }).strict();
var CourseDefinition = external_exports.object({
  id: courseIdSchema,
  title: external_exports.string().min(1),
  language: external_exports.string().min(2),
  // BCP-47; deep validation deferred
  estimatedMinutes: external_exports.number().positive(),
  objectives: external_exports.array(CourseObjective).min(1),
  opening: OpeningDefinition,
  parts: external_exports.array(PartDefinition).min(1),
  closing: ClosingDefinition
}).strict();
var CourseDefinitionDocument = external_exports.object({ schemaVersion: external_exports.literal("2.0"), course: CourseDefinition }).strict();

// packages/course-contract/src/session.ts
var RuntimeSceneResult = external_exports.object({
  text: external_exports.string(),
  audioUrl: external_exports.string().optional(),
  generatedAt: external_exports.string(),
  usedSignalTypes: external_exports.array(external_exports.string()),
  fallbackUsed: external_exports.boolean()
}).strict();
var BlockSessionState = external_exports.object({
  ...
  answer: external_exports.unknown().optional(),
  mediaPositionSeconds: external_exports.number().nonnegative().optional(),
  interactionResult: external_exports.unknown().optional()
}).strict();
var SliceSessionState = external_exports.object({
  status: external_exports.enum(["not-started", "in-progress", "completed"]),
  ...
  elapsedSeconds: external_exports.number().nonnegative(),
  blockStates: external_exports.record(blockIdSchema, BlockSessionState)
}).strict();
var CourseRuntimeEvent = external_exports.object({
  ...
  type: external_exports.union([WorkflowEventType, external_exports.string()]),
  occurredAt: external_exports.string(),
  payload: external_exports.unknown()
}).strict();
var CourseSession = external_exports.object({
  id: external_exports.string(),
  courseId: courseIdSchema,
  courseSchemaVersion: external_exports.literal("2.0"),
  studentId: external_exports.string(),
  status: external_exports.enum(["created", "opening", "in-progress", "closing", "completed"]),
  startedAt: external_exports.string().optional(),
  completedAt: external_exports.string().optional(),
  current: external_exports.object({ partId: partIdSchema, sliceId: sliceIdSchema, workflowStepId: workflowStepIdSchema }).strict().optional(),
  opening: RuntimeSceneResult.optional(),
  sliceStates: external_exports.record(sliceIdSchema, SliceSessionState),
  events: external_exports.array(CourseRuntimeEvent),
   ...
   */
  courseDefinitionHash: external_exports.string().optional()
}).strict();

// packages/course-contract/src/validate/referential.ts
function validateReferential(course) {
  const issues = [];
  const add = (path, message) => issues.push({ path, message, layer: "referential" });
  const seen = (items, key, kind, path) => {
  ...
  sliceEntries.forEach(({ slice, path }) => {
    ...
    slice.blocks.forEach((b, bi) => {
      if (b.type === "singleChoice" && b.assessment.mode === "graded") {
        const opts = new Set(b.options.map((o) => o.id));
...
// packages/course-contract/src/validate/workflow.ts
var VIDEO_ONLY_ACTIONS = /* @__PURE__ */ new Set(["playBlock", "pauseBlock", "resetBlock"]);
var BLOCK_TARGET_ACTIONS = /* @__PURE__ */ new Set(["show", "hide", "enable", "disable", "playBlock", "pauseBlock", "resetBlock"]);
var NARRATION_ACTIONS = /* @__PURE__ */ new Set(["playNarration", "pauseNarration", "stopNarration"]);
var INTERACTION_ID_EVENTS = /* @__PURE__ */ new Set(["video.interaction.shown", "video.interaction.completed", "interaction.completed"]);
var EVENT_PRODUCERS = {
...
function validateSliceWorkflow(slice, path) {
  const issues = [];
  const add = (p, message) => issues.push({ path: p, message, layer: "workflow" });
  ...
  for (const b of slice.blocks) blockById.set(b.id, b);
  ...
  const imageItemIds = /* @__PURE__ */ new Map();
  for (const b of slice.blocks) {
    if (b.type === "images") imageItemIds.set(b.id, new Set(b.items.map((it) => it.id)));
  }
  ...
  const declaredTimerIds = /* @__PURE__ */ new Set();
  ...
  steps.forEach((step, si) => {
    ...
    const actions = step.enterActions;
    ...
    step.transitions.forEach((t, ti) => {
      ...
      if (on.sourceId !== void 0 && producer) {
      ...
      if (on.interactionId !== void 0 && !INTERACTION_ID_EVENTS.has(on.type)) {
        add(onPath, `interactionId is not meaningful for event '${on.type}'`);
      }
      if (on.timerId !== void 0 && !declaredTimerIds.has(on.timerId)) {
        add(onPath, `timerId references timer '${on.timerId}' that is never started`);
      }
    });
    const trans = step.transitions;
  ...
  const isTerminal = (step) => step.enterActions.some((a) => a.type === "completeSlice" || a.type === "navigate");
  const canReachTerminal = /* @__PURE__ */ new Map();
  const reachesTerminal = (id, stack) => {
  ...
  steps.forEach((step, si) => {
    if (reachable.has(step.id) && !reachesTerminal(step.id, /* @__PURE__ */ new Set())) {
      add(`${wfPath}.steps[${si}]`, `step '${step.id}' cannot reach a completion or navigation step`);
    }
  });
  const hasNavigateAction = (step) => step.enterActions.some((a) => a.type === "navigate");
  for (const block of slice.blocks) {
...
function matchersOverlap(a, b) {
  ...
  for (const f of ["sourceId", "interactionId", "timerId"]) {
    if (a[f] !== void 0 && b[f] !== void 0 && a[f] !== b[f]) return false;
  }
  return true;
}
function completionEventTypesFor(block) {
  switch (block.type) {
    case "video":
      return block.completion ? /* @__PURE__ */ new Set(["block.completed"]) : null;
    case "interactiveHtml":
      return block.completion?.rule === "interaction-complete" ? /* @__PURE__ */ new Set(["interaction.completed", "block.completed"]) : null;
    case "singleChoice":
    case "fillBlank":
      return /* @__PURE__ */ new Set(["answer.correct", "answer.attemptsExhausted", "answer.submitted", "block.completed"]);
    default:
      return null;
  }
}
function sccHasSelfLoop(step, id) {
  if (!step) return false;
  return step.transitions.some((t) => t.to === id);
}
function stronglyConnectedComponents(steps, stepIds) {
...
// packages/course-contract/src/validate/quality.ts
var MAX_ASSETS = 256;
...
function validateQuality(document) {
  const { course } = document;
  const issues = [];
  const add = (path, message, severity = "error") => issues.push({ path, message, layer: "quality", severity });
    ...
    part.slices.forEach((slice, si) => {
      ...
      slice.blocks.forEach((block, bi) => {
        ...
        if (block.type === "images") {
          const counts = /* @__PURE__ */ new Map();
          block.items.forEach((item) => counts.set(item.id, (counts.get(item.id) ?? 0) + 1));
        ...
        if (block.type === "singleChoice") {
          const counts = /* @__PURE__ */ new Map();
          block.options.forEach((opt) => counts.set(opt.id, (counts.get(opt.id) ?? 0) + 1));
...
// packages/course-contract/src/validate/index.ts
function validateCourseDefinition(input) {
  const parsed = CourseDefinitionDocument.safeParse(input);
  if (!parsed.success) {
    return {
      ok: false,
      issues: parsed.error.issues.map((i) => ({ path: i.path.join("."), message: i.message, layer: "structural" }))
    };
  }
  const course = parsed.data.course;
  const issues = [...validateReferential(course), ...validateQuality(parsed.data)];
  course.parts.forEach(
    (part, pi) => part.slices.forEach((slice, si) => issues.push(...validateSliceWorkflow(slice, `parts[${pi}].slices[${si}]`)))
  );
