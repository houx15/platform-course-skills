#!/usr/bin/env python3
"""Manage the teacher-approved teaching design and Part/Slice page plan."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage a teacher-approved course teaching design and page plan")
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "approve":
            ...
            print(f"教学设计和逐页计划已审批：{approval['decisionId']}")
            return 0

        issues = validate_plan_at_root(args.root)
        if issues:
            print("状态：blocked。请先修复教学设计、逐页计划或课程素材证据。")
            return 2
        try:
            verify_plan_approval(args.root)
        except PlanApprovalError as exc:
            if exc.code == "approval-missing":
                print("状态：ready-for-approval。请在确认完整教学设计和逐页计划后执行 approve 并提供 decision ID。")
                return 0
            if exc.code == "approval-stale":
                print("状态：approval-stale。教学设计、逐页计划或素材证据已变化，请重新审批。")
                return 1
            print("状态：blocked。审批记录无效，请修复后重新审批。")
            return 2
        print("状态：approved-current。教师审批与当前教学设计、逐页计划和素材证据一致。")
        return 0
