...
静态检查通过后，Agent 会先用真实学生端 renderer 打开检查模式。这个模式允许直接跳到每个 Slice，不需要先答题或完成视频/HTML。Agent 保存并实际查看每页截图，像审一组教学幻灯片一样检查：单页内容密度与画面焦点是否合适，是否能看出与上一页的联系和整体框架中的位置，当前教学目的是否清楚，以及学生是否能理解为什么现在要做这件事；同时检查明显拥挤、空分栏、文字或媒体太小、图片和题目错配、题目引用的资料不在当前页、控件被裁切以及运行错误。所有弹窗内容都会实际打开一次；交互 HTML 还会检查完整活动，而不只看第一屏，确认在 1280×720 和较矮的 1200×520 画面中都能看到或滚动到主要任务、反馈、未完成提示和完成按钮。发现明显问题时先修改并重新检查，再把链接交给老师；不要求老师观看这个内部修复过程。
...
检查通过后，Agent 会打开本机预览链接。这里使用固定在 `course-authoring-v1.8.0` 的真实学生端 renderer；它包含 PDF 放大阅读、图片放大、结构化静态 `richText`、整槽交互 HTML，以及所有 Block 都可使用的近全屏弹窗。弹窗只改变呈现方式，不改变题目、学习数据或 workflow。开场、结尾和逐页讲解在本地预览中可以静音。`127.0.0.1` 是本地地址，这个链接只能在老师当前这台电脑上观看。老师完成本地预览或终审后，Agent 必须主动提醒：需要同事或其他老师预览时，下一步是在明确批准后发布到学生端并分享学生端页面；不要发送 ZIP 包、课程文件夹或本地链接。
...
1. 从课程开场开始；
2. 访问每一个 Slice；
3. 实际操作相关视频弹题、HTML、PDF、答题和翻页，并打开页面中的资料、图片、视频、HTML 或题目弹窗；
4. 检查整屏、弹窗和左右分栏是否清楚；交互 HTML 要走到活动末尾，确认反馈、未完成说明和完成按钮都能到达；旧课程若使用上下分栏，也要确认没有形成难读的横向薄条；
...
任何真实输入、Blueprint、素材或批注变化都会让受影响的后续结果重新生成。`teachingDesign`、学生视角预审和 Slice 语义连贯性审查只要求新课程或仍在重新设计的课程采用；旧课程不会仅因缺少后来新增的工作记录而失效，也不会被要求重新完成已经确认的教学问题。v1.8.0 只增加了可选的弹窗呈现和 HTML 画面提示，旧 JSON 仍然有效；学生端发布接口和发布逻辑没有改变。
...
支持的 Block 包括短文本、结构化静态 `richText`、图片组、PDF、视频、交互 HTML、填空和单选。每种 Block 都可以直接显示，也可以留一个清楚的按钮，按下后在近全屏弹窗中打开。页面的核心讲解、主要证据和理解任务所需的上下文要直接显示；原文、细节图、可回看的视频、较大的 HTML 活动、补充参考或会挤压资料的题目适合使用弹窗。弹窗不能用来隐藏学生必须先看到的说明。

短段落继续用 `text`；课程总览、当前方法位置、重要概念、方法步骤、带标注的示范、对照关系、证据阶梯、定义组、表格、量规和总结卡等需要清晰信息层级的静态讲解优先使用 `richText`。`richText` 还可以用克制的 HTML/CSS 卡片、连线、网格和步骤轨道表现流程、层级、因果链、二维矩阵或关系图，让知识结构本身可见。它不能只是一个标题加普通列表：应包含至少两个有明确教学作用的区域，例如带大数字的方法步骤、不同浅色的概念卡、当前步骤轨道、示范或 hint block。没有真实结构时应使用 `text`，不要把稀薄内容硬撑成满屏卡片。`richText` 只负责展示，不能接受输入或完成 Slice；老师确认教学设计和逐页计划后，Agent 才会调用内部的 `design-course-rich-text` specialist 生成安全的内联 HTML/CSS，不会借美化之名改写已确认的方法、案例、事实或题目。一个 Slice 应保持视觉可读，素材过多时拆页。课程结尾由学生数据与预先准备好的 fallback 总结共同支持，静态文本不得声称某个学生已经掌握内容。

开场、结尾与逐页 narration 的音频路径属于学生端 ship 阶段生成的 TTS 目标，不要求教师本地生成，也不会作为老师素材上传。当前本地预览使用 fallback 文本且静音。

## 媒体与交互检查

- 左右分屏强默认 `1:1`，两侧内容由共享 renderer 垂直居中。PDF 作为竖向页面在自己的栏位内居中显示，不因为是 PDF 就偏离 `1:1`；学生可以点击 PDF 标题栏的“放大阅读”，在接近全屏的弹窗中查看原文。只有“大视频 + 少量辅助文字”可以使用非对称比例，并把更宽一侧给视频。
- 交互 HTML 的 `1:1`、`4:3` 或 `fill` 只说明它原本偏好的画面形状。学生端会把整个栏位交给 HTML，HTML 自己要使用完整宽高并在内容较长时内部滚动；不得再套一个固定比例且裁切的画布，也不得把固定 1024×768 画布从左上角缩放后造成内容移出屏幕。制作和自检时同时检查 1280×720、1200×520 和弹窗打开状态。
- 弹窗适合需要放大的辅助资料或交互。主要教学内容保持在当前页直接可见；按钮文字要能说明会打开什么。题目使用弹窗时仍遵守原 workflow，资源弹窗只在学生点击后打开。
