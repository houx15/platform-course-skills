...
2. **开始一门新课程**：使用唯一教师入口 `build-platform-course`，按“理解材料 → 确认逐页计划 → 制作并自检 → 预览并发布”推进。
3. **继续已有课程**：读取当前 `.course-work` 和可用课程文件，告诉老师上次停留阶段、未解决问题和下一步；不要从头重新生成，也不要因为课程早于新检查流程就要求返工。
...
- 先介绍四阶段流程；完整盘点材料并生成逐页计划，请老师一次确认后，连续工作到首次完整预览；
...
- 逐页计划明确每页的教学目的、素材、学生行动、参考关系和大致 layout，并说明全部素材的使用或排除去向；
- 计划确认后，自行生成课程目标、课程首尾、完整 Part/Slice、layout、Blocks、narrations、workflow、导航和完成规则；
...
老师不需要选择或理解客户端的内部工作模式。正常情况下，Agent 先盘点材料并给出一份逐页教学计划；老师确认这份计划后，Agent 持续处理到课程可以预览。只有缺少必要文件、材料互相冲突、正确答案无法确定等真正阻塞项才会额外询问。
...
请先完整盘点材料并给我一份逐页计划，说明每页的教学目的、素材、学生行动和大致排版，也说明没有使用的素材及原因。我确认计划后，请自动完成 layout、workflow、格式检查、逐页视觉自检和首次预览。除非遇到无法从材料判断的真正阻塞项，不要反复让我确认。未经我明确批准，不要上传或提交课程。
```
...
Agent 先提交一份老师可读的逐页计划。一行对应一个 Slice，至少说明教学目的、使用素材、学生看到什么、学生做什么、参考材料与题目的对应关系，以及大致 layout；同时列出每份素材是使用、保留备用还是排除。老师一次确认整份计划，或指出需要调整的页面。老师不审核 JSON、稳定 ID、workflow 细节或 validator。
...
这些情况会合并成尽可能少的问题。其余事项由 Agent 先做初稿并在真实预览里一起判断。老师确认计划时主要检查：
...
计划确认后，老师不再逐项审批课程首尾文案、layout 或 workflow。Agent 完成制作并逐页截图自检后，老师在真实 renderer 预览中重点检查：
...
4. 生成完整逐页计划；一行对应一个 Slice，说明教学目的、素材、学生行动和大致 layout。老师一次确认这份计划。
5. 计划确认后，AI 补齐每个 Slice 的明确 layout、Blocks、narrations、workflow、导航和完成规则。
...
8. AI 先用真实学生端 renderer 的检查模式直接打开每个 Slice、截图并查看，修掉明显排版和内容错配后再给老师预览。
9. 教师可以在检查模式中跳过未作答内容，逐页查看并在侧栏写结构化批注。`apply-preview-feedback` 处理批注后重新编译、校验、AI 自检和预览。
10. 没有待处理批注时，AI 主动询问是否发布。教师确认后按原有发布逻辑上传必要 OSS 素材，保存或更新同一稳定 slug 的课程，并按要求 ship。

课程会绑定到正式的 33 门课程目录。老师只确认课程名称；绑定记录固定决定 slug、封面 OSS key、短介绍、七类分类、结构化介绍和工具卡，旧课程也不能跳过。封面不进入教师确认或教师素材上传流程。

任何真实输入、Blueprint、素材或批注变化都会让受影响的后续结果重新生成。旧课程不会仅因缺少后来新增的工作记录而失效。
