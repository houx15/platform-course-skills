...
正式发布前，Agent 还会安排一个独立的封面生成任务：由子 Agent 调用 imagegen2，根据已经确认的课程名称和介绍生成无可读文字、无 logo、无水印的视觉封面，并另存为 16:9、质量 100 的 WebP 候选。老师会直接看到图片并确认采用；原始生成结果和未采用候选都保留，确认绑定文件哈希，文件变化后必须重新看图确认。采用后的发布副本保存在 `.course-work/cover-delivery/course-cover.webp`，并以 `cover/course-cover.webp` 的课程相对路径按课程 ID 和文件哈希参与 OSS 上传与复用，不会污染 CourseDefinition 的正文素材集合。
