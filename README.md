...
检查通过后，Agent 会打开本机预览链接。这里使用固定在 `course-authoring-v1.5.2` 的真实学生端 renderer；它包含 v1.5.1 的 PDF 弹窗、加载反馈与报告滚动能力，并修复了 PDF 外部打开时占用课程标签页的问题。开场、结尾和逐页讲解在本地预览中可以静音。`127.0.0.1` 是本地地址，这个链接只能在老师当前这台电脑上观看。老师完成本地预览或终审后，Agent 必须主动提醒：需要同事或其他老师预览时，下一步是在明确批准后发布到学生端并分享学生端页面；不要发送 ZIP 包、课程文件夹或本地链接。
