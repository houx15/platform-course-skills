...
在老师看到它们以前，Agent 会先以学生视角完整走一遍，检查学生是否知道为什么学、方法是否先讲解和示范、练习是否有支架、页面是否像连续考试、是否持续形成一份学习成果、最后能否迁移。发现问题会先修改设计和计划，不把内部返工交给老师。
...
6. AI 在交给老师前代表学生完整走查，检查先教后练、示范与支架、答题负担、累计进展、动机节奏和迁移；先自行修订，再让老师一次确认完整教学设计和逐页计划。
...
任何真实输入、Blueprint、素材或批注变化都会让受影响的后续结果重新生成。`teachingDesign` 和学生视角预审只要求新课程或仍在重新设计的课程采用；旧课程不会仅因缺少后来新增的工作记录而失效，也不会被要求重新完成已经确认的教学问题。
