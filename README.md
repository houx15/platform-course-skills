...
请先完整盘点材料，先做一份以核心方法论为主线的教学设计，再从它生成逐页计划。教学设计要说明核心问题、方法步骤、案例练习、讲解与示范、支架、累计学习成果和迁移任务；逐页计划要说明每页对应的教学阶段、方法步骤、学生理解变化、素材、行动和大致排版，也说明没有使用的素材及原因。交给我之前，请先从学生视角完整审查一次，避免把课程做成连续答题。我一次确认整体设计和计划后，请自动完成 layout、workflow、格式检查、逐页视觉自检和首次预览。除非遇到无法从材料判断的真正阻塞项，不要反复让我确认。未经我明确批准，不要上传或提交课程。
```
...
Agent 先完成教学设计，再生成逐页计划。教学设计说明核心问题、学生起点与终点、核心方法及步骤、案例、学习路径、累计学习成果和迁移任务。逐页计划一行对应一个 Slice，至少说明对应教学阶段和角色、方法步骤、理解变化、使用素材、学生看到什么、学生做什么、参考材料与题目的对应关系，以及大致 layout；同时列出每份素材是使用、保留备用还是排除。
...
静态检查通过后，Agent 会先用真实学生端 renderer 打开检查模式。这个模式允许直接跳到每个 Slice，不需要先答题或完成视频/HTML。Agent 保存并实际查看每页截图，重点找明显拥挤、空分栏、文字或媒体太小、图片和题目错配、题目引用的资料不在当前页、控件被裁切以及运行错误。发现明显问题时先修改并重新检查，再把链接交给老师；不要求老师观看这个内部修复过程。
...
老师可以选择按正常 Workflow 体验课程，也可以使用检查模式。检查模式顶部有独立的“上一页 / 下一页”，即使题目、视频或 HTML 尚未完成也能直接翻到其他 Slice；这只服务于本地审阅，不会修改学生端完成规则。批注栏仍可打开使用。
...
7. 看完后点击“确认我已完整审查”。批注仍可保持待处理、标记完成、删除或重新打开；老师标记已完成的批注不会被系统擅自改回未解决。
...
支持的 Block 包括短文本、结构化静态 `richText`、图片组、PDF、视频、交互 HTML、填空和单选。短段落继续用 `text`；方法步骤、带标注的示范、对照关系、证据阶梯、定义组、表格、量规和总结卡等需要清晰信息层级的静态讲解可以用 `richText`。`richText` 只负责展示，不能接受输入或完成 Slice；老师确认教学设计和逐页计划后，Agent 才会调用内部的 `design-course-rich-text` specialist 生成安全的内联 HTML/CSS，不会借美化之名改写已确认的方法、案例、事实或题目。一个 Slice 应保持视觉可读，素材过多时拆页。课程结尾由学生数据与预先准备的 fallback 总结共同支持，静态文本不得声称某个学生已经掌握内容。
...
AI 交付预览前会先在检查模式访问每个 Slice 并逐页看截图。教师审阅时也可使用检查模式的独立翻页控制，不必先完成题目。批注可以继续保留、修改、删除、标记完成或重新打开；老师标记解决的内容不会被新检查自动重开。修改后必须重新生成并预览当前版本。
