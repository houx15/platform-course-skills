...
如果课程名称与正式的 33 门课程目录相近，Agent 会直接说“我判断最可能是《……》，请确认”。这只是推荐，不能静默选中。老师可以一开始确认，也可以回答“先做预览，提交前再定”。即使课程是用旧版工具做的，最终提交前也必须选定一门；随后 `category`、学生端结构化课程介绍和对应工具卡会自动取得，不需要老师编辑 JSON。
