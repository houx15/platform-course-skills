...
- 所有分栏槽位都必须非空。左右或上下分栏的每一侧都必须实际承载 Block；不允许把 2–3 个 Block 全堆在一侧，再用 `3:1` 留出空栏。一个聚焦内容使用 `full`；说明与作答并列时左右分配；连续堆叠的内容拆成多个 Slice。这条规则在课程整理阶段和 G6 都会阻止错误进入预览。
