...
2. **开始一门新课程**：使用唯一教师入口 `build-platform-course`，先盘点原始材料和待确认事项，再按 G0–G10 推进。
...
- 在材料盘点时明确询问视频和独立 HTML 的使用意图，即使现有材料没有提到它们；
- 先让老师确认教学目标、材料用途、课程首尾和完整 Part/Slice 设计，再生成完整 layout、Blocks、narrations、workflow、导航和完成规则；
- 自行编译、验证、启动真实 renderer 预览，并引导老师逐页操作和写批注；
- 读取预览批注，将机械修改、教学语义修改和学生端运行问题分开处理；涉及教学含义的修改必须重新确认；
