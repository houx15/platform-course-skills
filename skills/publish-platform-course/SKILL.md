...
5. For `publish`, require `.course-work/course-cover.json`. A separate subagent must call imagegen2 from the confirmed course title/introduction; preserve the original generation under `.course-work/cover-sources/` and use `manage-course-cover.py prepare` to create an exact 16:9 quality-100 WebP candidate through `cwebp -q 100`. Show it to the teacher and record explicit confirmation with `manage-course-cover.py confirm` before `.course-work/cover-delivery/course-cover.webp` enters the OSS manifest as `cover/course-cover.webp`. Hash changes require review again.
