...
22. The current implementation boundary begins at G7. Preview annotations belong only in `.course-work/`; annotations invalidate G8 and G9 until a rebuilt preview verifies them. The student renderer, browser preview, and annotation UI are not part of the current local implementation, so do not claim G7 from static files alone.
