...
23. Run `python scripts/manage-annotations.py prepare ROOT .course-work/annotation-revision-plan.json --json`. Present every proposed semantic change to the teacher. Only after the teacher explicitly approves the exact proposal and gives a rationale, run `python scripts/course-workflow.py confirm-decision ROOT DECISION_ID --choice approve --rationale "TEACHER_RATIONALE" --json`. Never infer approval. Then run `python scripts/manage-annotations.py apply ROOT .course-work/annotation-revision-plan.json --json`. This atomically updates Blueprint and annotation state, never `course/course.json`. Follow its mandatory sequence: reconcile, compile, complete G5, validate CourseDefinition 2.0, and complete G6. Applied annotations remain unverified; only a future real renderer preview at G7 may mark them verified against the rebuilt definition hash.
24. The current implementation boundary begins at G7. Preview annotations belong only in `.course-work/`; annotations invalidate G8 and G9 until a rebuilt preview verifies them. The student renderer, browser preview, and annotation UI are not part of the current local implementation, so the local CLI refuses manual G7 completion and static files cannot claim it.
25. After genuine renderer-backed G7 and independent G8 evidence exist, prepare publication locally. Initialize the stable identity once with `python scripts/prepare-publication.py init-state ROOT --slug SLUG --json`; it must refuse to replace a different existing `courseLocalId` or slug. Build the content-addressed manifest with `python scripts/prepare-publication.py manifest ROOT --json`. Accept remote discovery and review evidence only from the future trusted adapters, then run `python scripts/prepare-publication.py preflight ROOT --discovery DISCOVERY_JSON --review-evidence REVIEW_EVIDENCE_JSON --intended-status preview-or-published --visibility private-or-unlisted-or-public --json`. Present the teacher-readable create/update mode, same remoteCourseId for updates, expected revision, upload/reuse counts, intended status and visibility, and the fact that no remote write occurred. Never show credentials, object keys, or raw payloads in the ordinary summary.
26. Publication approval is exact and context-hashed in `.course-work/publication-preflight.json`. Obtain an explicit choice and rationale through `course-workflow.py confirm-decision`; `prepare-publication.py status` must still report the same dry run as current and approved. A changed definition, G6/G8 evidence, asset manifest, remote discovery, local identity, expected revision, status, or visibility requires a new preflight and approval. The local CLI cannot complete G9, because it cannot attest the live handoff. The mockable publisher uses hash-based uploads, optimistic revision checks, stable idempotency keys, and discovery/read-back before retry, with no second blind create. It has no teacher-facing execute command: fake adapters never complete G10. G10 requires a live OSS/course-API adapter operation and verified post-write remote read.
27. The current implementation has no OSS adapter or real course POST. Every external handoff requires explicit publication approval. G10 requires the real publication adapter. Local preparation may create manifests, dry runs, decisions, and fake-adapter test state only. Local completion never implies upload, POST, or publication. No external mutation follows from a request to build, revise, validate, or preview locally.
