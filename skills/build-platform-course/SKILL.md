...
20. The current implementation boundary after G5 is explicit: enhanced CourseDefinition 2.0 media/completeness validation is still being added. The legacy `review-platform-course` validator targets 1.1 and must not certify a 2.0 definition. Apply safe mechanical fixes to Blueprint; send pedagogical or semantic problems back through revised course-frame and Part/Slice tables and obtain teacher confirmation before recompiling.
21. Run the applicable full Review again after every rebuild. Before every required 2.0 check returns clear, 不得报告可上传 and do not complete G6.
22. Preview annotations belong only in `.course-work/`; annotations invalidate G8 and G9 until a rebuilt preview verifies them. The student renderer, browser preview, and annotation UI are not part of the current local implementation, so do not claim G7 from static files alone.
23. Publication always requires a dry run and explicit publication approval. The current implementation has no OSS adapter or real course POST. The CLI cannot complete G10: G10 requires the real publication adapter and a post-write remote read; local completion never implies upload, POST, or publication. No external mutation follows from a request to build, revise, validate, or preview locally.
