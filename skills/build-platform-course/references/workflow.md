...
Checks: invoke `design-video-interactions` and `design-course-html` internally where required; preserve originals. HTML interaction semantics may be completed from source evidence. Video interaction semantics may only encode teacher-provided video questions; when the teacher supplied none, do not propose or generate a video interaction. Record unknowable correctness, provisional timing, or absent files as blockers.
