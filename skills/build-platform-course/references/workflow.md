...
Require `.course-work/course-catalog-selection.json` even for a legacy course made before catalog support. It must still match the pinned 33-course dictionary; its category, structured introduction, and card IDs are the only values sent in the v1.4.0 authoring envelope. For `publish`, also require a teacher-confirmed 16:9 quality-100 WebP cover generated by a bounded imagegen2 subagent. Obtain the exact generation input from `manage-course-cover.py prompt ROOT --json`; the subagent must use that pinned course-cover prompt unchanged, and `prepare` verifies it before accepting the candidate. The catalog, prompt, and cover hashes are approval evidence, so any of them changing invalidates G9.
