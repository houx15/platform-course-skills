...
- Review HTML source, current validation evidence, the iframe completion/student-data protocol, capabilities, and real iframe behavior. `aspectRatio` is only a design hint; confirm the document uses the complete host frame without a clipped inner canvas. Check 1280×720 and 1200×520, scroll through the whole activity, and verify that the task, feedback, unmet-requirement explanation, and completion control remain reachable. When `capabilities.audio` is true, verify host lifecycle audio control and user-gesture behavior. Text must follow the 16px / 14px rules. A stale or missing report/evidence blocks; 静态检查不能证明真实 iframe behavior.
- Review every `openAs: "modal"` Block in both states: the launcher must be clear in the Slice, and substantive rich text, media, or HTML must use a large share of the viewport rather than the narrow video-question card width. Preserve completion behavior and a working close path. Record a small fixed dialog as a renderer blocker instead of redesigning the course around it. Primary teaching context cannot exist only inside a supporting modal. Modal presentation does not excuse an overcrowded or semantically incomplete base Slice.
