...
3. Ask the teacher to traverse every Slice at a desktop viewport, exercise meaningful branches and interactions, and inspect the runtime diagnostics panel. For the pinned v1.5.2 renderer, explicitly verify the first-entry loading surface is not a blank white screen; every PDF's 放大阅读 action opens a near-fullscreen modal and Esc, close button, and backdrop dismiss it; any external-open action opens a new tab without replacing the course; leaving the last Slice immediately shows `正在生成课程小结` without accepting a second click; completing the course shows `正在生成报告`; and the final report page scrolls through all content. Merely opening the URL is not teacher completion.
