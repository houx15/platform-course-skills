...
2. Start inspection mode:

   ```bash
   python _course-toolkit/scripts/preview-course.py ROOT --inspection
   ```

   In a repository checkout, use `python scripts/preview-course.py ROOT --inspection`.
...
1. Open the reviewed course. Use normal preview when the teacher wants to experience the authored Workflow exactly; use `--inspection` when the teacher wants to move freely between pages without answering first.
...
When the teacher finishes preview and no annotation is pending, proactively ask: `现在发布到学生端吗？`
