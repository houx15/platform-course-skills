...
4. Summarize evidence with source file, stable source ID, and original location. Preserve DOCX table row/cell structure. Separate concepts, transferable methodologies, method steps, worked examples, cases, evidence, activities, questions, answers, feedback, misconceptions, sequence cues, cumulative learner artifacts, media notes, teacher notes, and system rules.
5. Identify duplicate claims, conflicts, unsupported claims, and missing information that would change the learning goal, correct answer, feedback, or media behavior. Specifically extract any teacher-provided `课程目标`, `课程总结`, `学生收获`, named framework, teaching method, worked model, practice sequence, and transfer task. When an HTML or other source already contains a coherent teaching journey, treat that sequence as primary teaching-design evidence rather than merely an asset to embed or a bag of questions. Preserve source IDs and 区分教师意图与学生措辞: the former is authoring evidence, while the latter must later be rewritten as concise learner-facing prose and bullets. If one is absent but a reasonable version follows from the content, record it as AI-proposed rather than confirmed. Ask only when no reliable learning purpose or correctness can be inferred.
...
8. Persist a concise grouped summary with `teacherConfirmed: false`. Return it to the director so it can first build the methodology-centred teaching design and then derive the complete page plan. Do not ask the teacher to confirm inventory categories separately; the teacher will confirm the combined teaching design, Part/Slice plan, material placement, and exclusions once.
